...
console.log("Pool MySQL initialisé (via src/config/db.js)");
...
app.post("/api/trucks", async (req, res) => {
  ...
  if (!entrepotId || !immatriculation || !transporteur) {
    return res
      ...
      .json({
        message:
          "Champs obligatoires manquants (entrepotId, immatriculation, transporteur)",
      });
  }
  ...
  try {
    ...
    if (warehouseCheck.length === 0) {
      return res
        .status(400)
        .json({ message: "Entrepôt invalide ou inexistant" });
    }
  } catch (e) {
    console.error("Validation Error:", e);
    return res
      .status(500)
      .json({ message: "Erreur validation entrepôt", error: e.message });
  }
...
app.put("/api/trucks/:id", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query("SELECT metadata FROM trucks WHERE id = ?", [
      id,
    ]);
    if (rows.length === 0) {
    ...
    if (
      statut &&
      (statut === "Validé" || statut === "Refoulé" || statut === "Annulé")
    ) {
    ...
    if (transporteur !== undefined) {
      fields.push("transporteur=?");
      values.push(transporteur);
    }
    if (transfert !== undefined) {
      fields.push("transfert=?");
      values.push(transfert);
    }
    if (cooperative !== undefined) {
      fields.push("cooperative=?");
      values.push(cooperative);
    }
    if (statut !== undefined) {
      fields.push("statut=?");
      values.push(statut);
    ...
    if (heureDepart !== undefined) {
      fields.push("heureDepart=?");
      values.push(heureDepart);
    }
...
app.get("/api/users/:id", async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      "SELECT id, nom, username, role, entrepotId, status FROM users WHERE id = ?",
      [id],
    );
    console.log(
      `[DEBUG] DB Result for user ${id}:`,
      rows.length > 0 ? "Found" : "Not Found",
    );
    if (rows.length === 0) {
...
app.put("/api/users/:id", async (req, res) => {
  ...
  try {
    ...
    let query =
      "UPDATE users SET nom=?, username=?, role=?, entrepotId=?, status=?";
...
app.use((err, req, res, next) => {
  if (err instanceof multer.MulterError) {
    if (err.code === "LIMIT_FILE_SIZE") {
      return res
        .status(400)
        .json({ message: "L’image est trop lourde (max 10 Mo)." });
    }
  }
...
app.get("/api/warehouses", async (req, res) => {
  try {
    ...
    fixedRows.forEach((r) =>
      console.log(`ID: ${r.id}, Name: ${r.name}, Img: ${r.imageUrl}`),
    );
    res.json(fixedRows);
  } catch (err) {
    console.error(err);
...
app.get("/api/warehouses/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const [rows] = await db.query("SELECT * FROM warehouses WHERE id = ?", [
      id,
    ]);
    if (rows.length === 0) {
...
app.post("/api/warehouses", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ id: result.insertId, name, location, imageUrl: finalUrl });
  } catch (err) {
    console.error(err);
...
app.get("/api/empotages/:id", async (req, res) => {
  ...
  try {
    ...
    const [containers] = await db.query(
      "SELECT * FROM empotage_containers WHERE empotageId = ? ORDER BY id ASC",
      [id],
    );
...
app.post("/api/empotages/init", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await connection.query(
        "INSERT INTO notifications (message, relatedId, type) VALUES (?, ?, ?)",
        [msg, empotageId, "empotage"],
      );
    } catch (e) {}
    ...
    const [rows] = await connection.query(
      "SELECT * FROM empotages WHERE id = ?",
      [empotageId],
    );
    const empotage = rows[0];
    const [containers] = await connection.query(
      "SELECT * FROM empotage_containers WHERE empotageId = ?",
      [empotageId],
    );
    ...
    res
      .status(500)
      .json({ message: "Erreur création empotage", error: err.message });
  } finally {
    connection.release();
  }
});
...
app.post("/api/empotages/:id/add-container", async (req, res) => {
  ...
  try {
    ...
    // Check status
    const [rows] = await connection.query(
      "SELECT status, conteneurs, volume FROM empotages WHERE id = ?",
      [empotageId],
    );
    ...
    await connection.query(
      "UPDATE empotages SET conteneurs = ?, volume = ? WHERE id = ?",
      [newCount, newVolume, empotageId],
    );
...
app.put("/api/empotages/:id/finalize", async (req, res) => {
  ...
  try {
    ...
    await db.query(
      "UPDATE empotages SET status = ?, dateEnd = ? WHERE id = ?",
      ["Terminé", dateEnd, id],
    );
...
app.delete("/api/empotages/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Erreur suppression empotage", error: err.message });
  }
});
...
app.put("/api/empotage-containers/:id", async (req, res) => {
  ...
  try {
    ...
    const [existing] = await connection.query(
      "SELECT empotageId, volume FROM empotage_containers WHERE id = ?",
      [id],
    );
    if (existing.length === 0) {
      connection.release();
    ...
    const [rows] = await connection.query(
      "SELECT SUM(volume) as totalVol FROM empotage_containers WHERE empotageId = ?",
      [empotageId],
    );
    const totalVol = rows[0].totalVol || 0;

    await connection.query("UPDATE empotages SET volume = ? WHERE id = ?", [
      totalVol,
      empotageId,
    ]);
...
app.get("/api/empotages/export", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="empotages-export.csv"',
    );
    res.send(csv);
  } catch (err) {
...
app.get("/api/empotages/:id/export", async (req, res) => {
  ...
  try {
    ...
    const csvRows = containers.map((c) =>
      [c.numeroConteneur, c.nombreSacs, c.poids, c.volume, c.createdAt]
        .map(escape)
        .join(","),
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="booking-${booking.booking}.csv"`,
    );
    res.send(csvContent);
  } catch (err) {
